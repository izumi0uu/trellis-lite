...
Authoritative current command reference for `trellis-lite channel` subcommands,
...
```
trellis-lite channel <subcommand>
```
...
```bash
trellis-lite channel create <name>
...
```bash
trellis-lite channel send <name> [text]
...
```bash
trellis-lite channel messages <name>
  [--scope project|global]
...
```bash
trellis-lite channel wait <name>
...
- Use `trellis-lite channel interrupt` (the command) only when you actually want
...
```bash
trellis-lite channel send T --as A --stdin < /tmp/message.md
trellis-lite channel send T --as A --text-file /tmp/message.md
```
...
```bash
trellis-lite channel interrupt <name> [text]
...
```bash
trellis-lite channel spawn <name>
  [--scope project|global]
  [--agent <agent-name>]                  # loads .trellis/agents/<name>.md
  [--provider codex]                      # overrides agent file
...
- Provider is validated against the adapter registry
  (`packages/cli/src/commands/channel/adapters/`); current: `codex`.
...
```bash
trellis-lite channel kill <name>
...
```bash
trellis-lite channel post <name> <action>
  --as <agent>                            # REQUIRED
  [--scope project|global]
...
```bash
trellis-lite channel forum <name>
  [--scope project|global]
  [--status <status>]
  [--raw]
```
...
```bash
trellis-lite channel thread <name> <thread-key>
  [--scope project|global]
  [--raw]

trellis-lite channel thread rename <name> <old-thread> <new-thread>
  --as <agent>                            # REQUIRED
  [--scope project|global]
```
...
```bash
trellis-lite channel context add <name>
...
trellis-lite channel context delete <name>
  [--as <agent>]                          # default: main
  [--scope project|global]
  [--thread <key>]
  [--file <abs-path>] ...
...
trellis-lite channel context list <name>
  [--scope project|global]
  [--thread <key>]
...
```bash
trellis-lite channel title set <name>
  --title <text>                          # REQUIRED
  [--as <agent>]                          # default: main
  [--scope project|global]

trellis-lite channel title clear <name>
  [--as <agent>]                          # default: main
  [--scope project|global]
```
