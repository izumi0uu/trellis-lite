 ...
 * `trellis-lite channel spawn --agent <name>`, which loads `.trellis/agents/<name>.md`
 ...
 * runtime. We surface that mismatch eagerly (at `trellis-lite init --workflow` /
 * `trellis-lite workflow --template` time) so users can run `trellis-lite update` before
 * the first spawn.
 *
 ...
 *
 *   1. `--agent <name>` flag on a `trellis-lite channel spawn ...` command
