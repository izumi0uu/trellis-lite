 ...
 * `python`) had `trellis-lite init` fail outright.
 *
...
The developer just ran \`trellis-lite init\` on this project for the first time.
...
any existing convention docs (AGENTS.md, CONVENTIONS.md, CONTRIBUTING.md,
...
function getBootstrapTaskJson(
  ...
  return emptyTaskJson({
    ...
    createdAt: today,
    relatedFiles,
    notes: `First-time setup task created by trellis-lite init (${projectType} project)`,
  });
}
...
function getJoinerTaskJson(developer: string, taskName: string): TaskJson {
  ...
  return emptyTaskJson({
    ...
    createdAt: today,
    notes:
      "Generated by trellis-lite init for a new developer joining an existing Trellis project",
  });
}
...
\`${developer}\` just ran \`trellis-lite init\` on a fresh clone, saw "Developer
...
Trellis Lite is a workflow layer for Codex and Oh My Pi that keeps agents
consistent with project-specific conventions instead of writing generic code.
...
"Welcome! Your \`trellis-lite init\` set me up to onboard you to this project. I
...
export async function init(options: InitOptions): Promise<void> {
  ...
  // into the trellis hash
  // manifest, and a subsequent `trellis-lite uninstall` would wipe it.
  ...
  if (explicitTools.length > 0) {
    ...
    // No explicit tools + -y: install both supported platforms.
  ...
  if (selectedTemplate) {
    ...
    if (result.success) {
      ...
      const retryCmd = registry
        ? `trellis-lite init --registry ${registry.gigetSource} --template ${selectedTemplate}`
        : `trellis-lite init --template ${selectedTemplate}`;
    ...
    if (result.success) {
      ...
      console.log(
        chalk.gray(
          `   You can retry later: trellis-lite init --registry ${registry.gigetSource}`,
        ),
      );
    }
  }
  ...
  // (.codex/sessions/, OMP session data, pre-existing AGENTS.md).
  ...
  // `.trellis/workflow.md` hash entry so `trellis-lite update` classifies it as
