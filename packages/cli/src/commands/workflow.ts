/**
 * `trellis-lite workflow` command — list and switch the active `.trellis/workflow.md`.
 *
 * Behavior contracts:
 *
 * - Hash boundary: after writing native content, refresh the
 *   `.trellis/workflow.md` entry in `.template-hashes.json`. After writing
 *   any non-native content, remove that entry. This prevents `trellis-lite update`
...
function applyHashContract(cwd: string, templateId: string): void {
  ...
  if (templateId === NATIVE_WORKFLOW_ID) {
    ...
    // so `trellis-lite update` treats it as modified and does not silently restore
    // native bytes.
...
export async function runWorkflowCommand(
  ...
  if (!fs.existsSync(path.join(cwd, DIR_NAMES.WORKFLOW))) {
    throw new WorkflowCommandError(
      "No .trellis/ directory found. Run `trellis-lite init` first.",
    );
  }
  ...
  // at `trellis-lite update` so `trellis-lite channel spawn --agent <name>` doesn't fail
...
function warnAboutMissingAgents(cwd: string, workflowContent: string): void {
  ...
  process.stderr.write(
    chalk.yellow(
      ...
      chalk.yellow(
        `  Run \`trellis-lite update\` to backfill the bundled agent definitions, or create them under ${PATHS.AGENTS}/.\n`,
      ),
  );
}
