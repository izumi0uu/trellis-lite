 ...
 *   1. Version match between `trellis-lite` and
 *      `trellis-lite-core` (and the current git tag when checked from
 *      a tag context).
...
function verifyPackedCli() {
  ...
  try {
    ...
    const dep = packedPkg.dependencies?.["trellis-lite-core"];
    if (!dep) {
      fail(`packed CLI is missing dependency on trellis-lite-core.`);
    }
    if (dep !== v.cliVersion) {
      fail(
        `packed CLI depends on trellis-lite-core@"${dep}" but expected exact "${v.cliVersion}".\n` +
          `pnpm should rewrite workspace:* to the exact published version; got "${dep}" instead.`,
      );
    }
    console.log(
      `${GREEN}ok${RESET} packed CLI pins trellis-lite-core to exact ${v.cliVersion}.`,
    );
  } finally {
