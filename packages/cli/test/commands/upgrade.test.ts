...
describe("upgrade command", () => {
  ...
  it("builds POSIX npm global install command without shell", () => {
    expect(
      ...
      command: "npm",
      args: ["install", "-g", "trellis-lite@beta"],
      spawnOptions: { stdio: "inherit", shell: false },
      displayCommand: "npm install -g trellis-lite@beta",
      target: "trellis-lite@beta",
      tag: "beta",
      binaryCheckCommand: "which trellis-lite",
    });
  });
  ...
  it("builds Windows command through cmd.exe", () => {
    expect(
      ...
      command: "cmd.exe",
      args: ["/d", "/s", "/c", "npm install -g trellis-lite@beta"],
      spawnOptions: { stdio: "inherit", shell: false },
      displayCommand: "npm install -g trellis-lite@beta",
      target: "trellis-lite@beta",
      tag: "beta",
      binaryCheckCommand: "where trellis-lite",
    });
  });
  ...
  it("dry-run does not execute npm", async () => {
    ...
    expect(log).toHaveBeenCalledWith(
      expect.stringContaining("Run: npm install -g trellis-lite@latest"),
    );
  ...
  it("executes npm install for real upgrades", async () => {
    ...
    expect(runner).toHaveBeenCalledWith(
      "npm",
      ["install", "-g", "trellis-lite@latest"],
      { stdio: "inherit", shell: false },
    );
    expect(log).toHaveBeenCalledWith(
      expect.stringContaining("trellis-lite --version"),
    );
    expect(log).toHaveBeenCalledWith(
      expect.stringContaining("which trellis-lite"),
    );
  ...
  it("fails when npm exits non-zero", async () => {
    ...
    await expect(upgrade({ tag: "latest" }, runner)).rejects.toThrow(
      /npm install failed with exit code 1\.[\s\S]*Troubleshooting:[\s\S]*Manual command: npm install -g trellis-lite@latest[\s\S]*npm config get prefix[\s\S]*which trellis-lite/,
    );
