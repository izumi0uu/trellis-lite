/**
 * Persisted Oh My Pi session reader.
 *
...
export function ompListSessions(f: MemFilter): MemSessionInfo[] {
...
export function ompExtractDialogue(s: MemSessionInfo): DialogueTurn[] {
  return buildPiTurnsAndEvents(s).turns;
}

export function ompSearch(s: MemSessionInfo, kw: string): SearchHit {
  return searchInDialogue(ompExtractDialogue(s), kw);
}

export function collectOmpTurnsAndEvents(s: MemSessionInfo): PiBuilt {
