...
`trellis-lite channel messages <channel>` renders a compact, human-readable view:
...
trellis-lite channel messages <channel> --kind done --last 10
trellis-lite channel messages <channel> --kind error --last 10

# Raw (diagnostic view) — one JSON per line
trellis-lite channel messages <channel> --raw --kind progress --last 20
trellis-lite channel messages <channel> --raw --last 50
```
...
```bash
trellis-lite channel messages <channel> --raw --kind progress --last 80 \
...
Symptom: `trellis-lite channel list` shows the worker as running, but no new
...
1. **Locate the channel files.** Use `list --all --all-projects` if you are
   ...
   ```bash
   trellis-lite channel list --all --all-projects
...
2. **Confirm the supervisor and worker PIDs are alive.**
   ...
   `trellis-lite channel kill <name> --as <worker> --force`.
...
```bash
trellis-lite channel wait T --as main --from check --kind done --timeout 15m
trellis-lite channel wait T --as main --from check,check-cx --kind done --all --timeout 15m
trellis-lite channel wait T --as worker --tag interrupt --timeout 1h
trellis-lite channel wait T --as main --thread release-note --action status --timeout 10m
```
...
trellis-lite channel forum list <channel>

# Inspect one thread end-to-end
trellis-lite channel thread show <channel> <thread>

# Replay messages for a thread (supports --raw, --kind, --last)
trellis-lite channel messages <channel> --thread <thread> --raw --last 100

# What a specific worker still has pending
trellis-lite channel context <channel> --as <worker>
```
...
| Symptom | Cause | Fix |
|---|---|---|
| `trellis: command not found` | CLI not installed globally | `npm install -g trellis-lite` |
...
| ghost worker in list | supervisor died without cleanup | `trellis-lite channel kill <name> --as <worker> --force` |
