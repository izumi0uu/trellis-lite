...
- **Pi Agent** uses its project-local TypeScript extension. `tool_call`
  invokes the shared Python entrypoint before built-in `write` and `edit`;
  a FULL emission returns `{ block: true, reason }`, which Pi records as a
  model-visible error tool result before continuing the agent loop. `read`
  invokes the provider from `tool_result`, after a successful file read.
  Non-blocking tickets from mutation preflight are appended to the matching
  tool result by `toolCallId`. The Python entrypoint lives at
  `.trellis/scripts/inject-spec-context.py`; Pi does not install Python hooks
  under `.pi/`.
...
`session.compacted` event to the same compact reset; Pi maps
`session_compact`. Which tool events trigger remains filtered in-hook by
...
adapter turns the same FULL decision into a model-visible thrown error, while
Pi returns the context as its model-visible blocked-tool reason.
...
| Pi Agent                                                                           | ✅ wired       | Pi 0.80.6 extension API verified 2026-07-29: built-in `write` / `edit` expose `input.path` in `tool_call`; `{ block: true, reason }` becomes a model-visible error tool result; `tool_result` can append Read context and non-blocking tickets; `session_compact` resets exposure. No Python hook is installed under `.pi/`.                                                        |
| cursor, gemini, qoder, copilot, codebuddy, droid, kiro, trae, zcode, omp, snow     | follow-up      | Register only after verifying the platform has a tool-event hook that consumes injected context and, for pre-tool hooks, can return it to the model before retry.                                                                                                                                                                                                                                                                               |
...
| Pi shared entrypoint missing or subprocess/JSON parsing fails              | extension lets the tool call proceed                                                                                                                                                                                   |
| Pi state unavailable                                                       | stateless ticket-only response is appended after the tool result; mutation is not blocked                                                                                                                              |
